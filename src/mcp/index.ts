...
export function registerMcpOperations(daemon: AERDaemon, options: Pick<McpSurfaceOptions, "github" | "agent"> = {}): AERDaemon {
  ...
  registerIfMissing(daemon, {
    ...
    execute: (input, context) => {
      ...
      const bytes = daemon.artifacts.read(value.ref, { ...(context.projectId === undefined ? {} : { projectId: context.projectId }), offset, length: maxBytes });
...
export function createMcpServer(options: McpSurfaceOptions = {}): McpServer {
  ...
  server.registerTool("run.inspect", { description: "Inspect bounded durable state for one AER run.", inputSchema: { projectId: z.string().optional(), runId: z.string().min(1), budgets: budgetSchema } }, async (input) => resultContent(daemon, await call(daemon, "run.inspect", input as Record<string, unknown>)));
